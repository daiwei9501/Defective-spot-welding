...
# %% -------------------------------------- Data Prep ------------------------------------------------------------------
labels = ['good', 'edge weld', 'double', 'pseudo welds', 'burr', 'distortion', 'burn through']
...
pretrained_model = load_model('./Resnet50model.h5')
...
# %% -------------------------------------- TSNE Visualization ---------------------------------------------------------
def tsne_plot(encoder, embedding):
    ...
    #latent = feature_model.predict(x_train)
    ...
    x_real = x[:-7 * aug_size]
    y_real = y[:-7 * aug_size]
    x_generated = x[-7 * aug_size:]
    y_generated = y[-7 * aug_size:]
    real_label = y_train[:-7 * aug_size]
    generated_label = y_train[-7 * aug_size:]
    loop = 0
    markers = ['o', 'x']
    for x, y, l in [(x_real, y_real, real_label), (x_generated, y_generated, generated_label)]:
        ...
        loop += 1
        for c in range(n_classes):
            plt.scatter(x[l == c], y[l == c], marker=marker, c=np.array([color(c)]), label=labels[c])
    plt.legend()
    plt.show()
